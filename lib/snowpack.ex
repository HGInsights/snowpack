defmodule Snowpack do
  ...
  Start connection using the default configuration (UNIX domain socket):
      ...
      iex> {:ok, pid} = Snowpack.start_link([])
      {:ok, #PID<0.69.0>}

  Start connection over TCP:

      iex> {:ok, pid} = Snowpack.start_link(protocol: :tcp)
      {:ok, #PID<0.69.0>}
  ...
  errors such as:
      ...
      ** (Snowpack.Error) (1792) (ER_CANT_EXECUTE_IN_READ_ONLY_TRANSACTION) Cannot execute statement in a READ ONLY transaction.

  Luckily, you can instruct Snowpack to disconnect in such cases by using the following configuration:
  ...
  Snowpack automatically disconnects the connection on the following error codes and they don't have
  to be configured:
  ...
  Snowpack supports MySQL's two ways of executing queries:
  ...
  ## Examples
      ...
      iex> Snowpack.query(conn, "CREATE TABLE posts (id serial, title text)")
      {:ok, %Snowpack.Result{}}

      iex> Snowpack.query(conn, "INSERT INTO posts (title) VALUES ('title 1')")
      {:ok, %Snowpack.Result{last_insert_id: 1, num_rows: 1}}

      iex> Snowpack.query(conn, "INSERT INTO posts (title) VALUES (?)", ["title 2"])
      {:ok, %Snowpack.Result{last_insert_id: 2, num_rows: 1}}
  ...
  ## Examples
      ...
      {:ok, result} =
        Snowpack.transaction(pid, fn conn  ->
          Snowpack.query!(conn, "SELECT title FROM posts")
        end)
  ...
  ## Examples
      ...
      {:ok, results} =
        Snowpack.transaction(pid, fn conn ->
          stream = Snowpack.stream(conn, "SELECT * FROM integers", [], max_rows: max_rows)
          Enum.to_list(stream)
        end)
  ...
  We'll get following results:
      ...
      Enum.at(results, 0)
      #=> %Snowpack.Result{num_rows: 0, ...}

      # The second item is result of fetching rows 1 & 2
      Enum.at(results, 1)
      #=> %Snowpack.Result{num_rows: 2, rows: [[1], [2]]}

      # The third item is result of fetching rows 3 & 4
      Enum.at(results, 2)
      #=> %Snowpack.Result{num_rows: 2, rows: [[3], [4]]}
  ...
  However, if the table contained only 3 rows, the 3rd result would contain:
      ...
      Enum.at(results, 2)
      #=> %Snowpack.Result{num_rows: 1, rows: [[3]]}

  And that would be the last result in the stream.
  """

  # @spec stream(DBConnection.t(), iodata | Snowpack.Query.t(), list, [stream_option()]) ::
  ...
  #   query = %Snowpack.Query{name: "", statement: statement, num_params: length(params)}
  #   opts = Keyword.put_new(opts, :max_rows, 500)
  #   DBConnection.prepare_stream(conn, query, params, opts)
  # end

  # def stream(%DBConnection{} = conn, %Snowpack.Query{} = query, params, opts) do
  #   opts = Keyword.put_new(opts, :max_rows, 500)
