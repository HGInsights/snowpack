defmodule Snowpack.Protocol do
  ...
  * `:conn_opts`: the options used to set up the connection.
  ...
  @type state :: %__MODULE__{
          pid: pid(),
          snowflake: :idle | :transaction | :auto_commit,
          conn_opts: Keyword.t()
        }
  ...
  def connect(opts) do
    ...
    {:ok, _pid} = Snowpack.TypeParser.TypeCache.start_link(pid)
  ...
  defp clean_result(result) do
    case result do
      {:ok, _query, %Snowpack.Result{} = result, %Snowpack.Protocol{} = state} ->
        {:ok, result, state}
  ...
  defp handle_transaction(:begin, _opts, state) do
    case state.snowflake do
      :idle ->
        {:ok, %Result{num_rows: 0}, %{state | snowflake: :transaction}}

      :transaction ->
        {:error, %Snowpack.Error{message: "Already in transaction"}, state}

      :auto_commit ->
        {:error, %Snowpack.Error{message: "Transactions not allowed in autocommit mode"}, state}
    end
  end

  defp handle_transaction(:commit, _opts, state) do
    case ODBC.commit(state.pid) do
      :ok -> {:ok, %Result{}, %{state | snowflake: :idle}}
  ...
  defp handle_savepoint(:begin, opts, state) do
    if state.snowflake == :autocommit do
      {:error, %Snowpack.Error{message: "savepoint not allowed in autocommit mode"}, state}
    else
      handle_execute(
        %Snowpack.Query{
          name: "",
          statement: "SAVE TRANSACTION snowpack_savepoint"
        },
        [],
        opts,
        state
  ...
  defp handle_savepoint(:rollback, opts, state) do
    handle_execute(
      %Snowpack.Query{
        name: "",
        statement: "ROLLBACK TRANSACTION snowpack_savepoint"
      },
      [],
      opts,
      state
  ...
  defp do_query(query, params, opts, state) do
    case ODBC.query(state.pid, query.statement, params, opts) do
      {:error, %Snowpack.Error{odbc_code: :not_allowed_in_transaction} = reason} ->
        if state.snowflake == :auto_commit do
          {:error, reason, state}
        else
